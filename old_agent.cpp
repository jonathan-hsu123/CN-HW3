...
int main(int argc, char *argv[]){
	...
	const struct sockaddr* RECV = (const struct sockaddr *) &recvaddr;
	const struct sockaddr* SEND = (const struct sockaddr *) &sendaddr;
	...
	srand(time(NULL));
	while(true) {
		seg_size = recvfrom(sockfd, &tmp_seg, sizeof(segment), 0, 0, 0);
		...
		if(seg_size > 0) {
			if(portfrom == ntohs(sendaddr.sin_port)) { //from sender
				...
				if(tmp_seg.head.fin == 1) { //fin
					printf("get     fin\n");
					sendto(sockfd, &tmp_seg, sizeof(segment), 0, RECV, sizeof(recvaddr));
					printf("fwd     fin\n");
				}
				else { //data
				...
				if(tmp_seg.head.fin == 1) { //fin ack
					printf("get     finack\n");
                    sendto(sockfd, &tmp_seg, sizeof(segment), 0, SEND, sizeof(sendaddr));
                    printf("fwd     finack\n");
					break;
				}
				else {
